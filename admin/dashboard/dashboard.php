...
if (isset($_POST['save'])) {
    ...
    if ($password != $cpassword) {
        $error['compare'] = "Password and Confirm Password should be same";
    }

    $passwordHash = password_hash($password, PASSWORD_DEFAULT);

    if (empty($error)) {
        $sqlUpdate = $conn->prepare("UPDATE tbluser SET password = '$passwordHash' WHERE id = '$id' ");
        $sqlUpdate->execute();
        $data = $sqlUpdate->get_result();
